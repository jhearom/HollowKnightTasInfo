...
namespace Assembly_CSharp.TasInfo.mm.Source {
    ...
    internal class AutoSplit : BaseTimer {
        ...
        public static void OnPreRender(GameManager gameManager, StringBuilder infoBuilder) {
            ...
            if (loadedGeneration != SplitReader.Generation) {
                loadedGeneration = SplitReader.Generation;
                currentSplitIndex = 0;
                SplitLastSplit = false;
                timeSincePause = 0f;
                timeSinceResume = 0f;
                didSplit = false;
                store.Reset();
            ...
            if (!SplitReader.ReadSplits || SplitReader.SplitList.Count == 0) {
                return;
            }
            ...
            if (sceneName != lastSceneName) {
                store.Reset();
                store.SetTraitorLordDeadOnEntry(gameManager.playerData?.killedTraitorLord ?? false);
                lastSceneName = sceneName;
            }

            if (currentSplitIndex >= SplitReader.SplitList.Count) {
                currentSplitIndex = SplitReader.SplitList.Count - 1;
            }
